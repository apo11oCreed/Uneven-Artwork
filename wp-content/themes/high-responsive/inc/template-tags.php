...
function highresponsive_footer_content() {
	...
	<div class="site-info two">
		<div class="wrapper">
			<div id="footer-left-content" class="copyright">' .  sprintf( _x( 'Copyright &copy; %1$s %2$s. All Rights Reserved. %3$s', '1: Year, 2: Site Title with home URL, 3: Privacy Policy Link', 'high-responsive' ), esc_attr( date_i18n( __( 'Y', 'high-responsive' ) ) ), '<a href="'. esc_url( home_url( '/' ) ) .'">'. esc_attr( get_bloginfo( 'name', 'display' ) ) . '</a>', get_the_privacy_policy_link() ) . '</div>

			<div id="footer-right-content" class="powered">' . $theme_data->get( 'Name' ) . '&nbsp;' . esc_html__( 'by', 'high-responsive' ) . '&nbsp;<a target="_blank" href="' . esc_url( $theme_data->get( 'AuthorURI' ) ) . '">' . esc_html( $theme_data->get( 'Author' ) ) . '</a></div>
		</div><!-- .wrapper -->
	</div><!-- .site-info -->';
}
